...
function TrendArrow({ current, previous }) {
  if (previous === null || previous === undefined || current === previous) {
    return <span className="text-xs text-gray-400 dark:text-gray-500 ml-1">&ndash;</span>
  }
  if (current > previous) {
    return (
  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 flex flex-col gap-1">
      ...
      <div className="flex items-baseline">
        ...
        <TrendArrow current={typeof value === "string" ? parseFloat(value) : value} previous={previous} />
      </div>
    </div>
  )
}
  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3">
      ...
      <div className="space-y-1.5 max-h-32 overflow-y-auto">
        {alerts.length === 0 && (
          <div className="text-xs text-gray-400 dark:text-gray-500">No alerts yet</div>
        )}
        {alerts.map((alert, i) => (
          <div key={i} className="flex items-start gap-2 text-xs">
            <span className={`px-1.5 py-0.5 rounded font-medium shrink-0 ${levelStyles[alert.level] || levelStyles.info}`}>
              {alert.level}
            </span>
            <span className="text-gray-700 dark:text-gray-300 flex-1">{alert.message}</span>
            <span className="text-gray-400 dark:text-gray-500 shrink-0">{alert.time}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  useEffect(() => {
    ...
    connectTokenFn().then((data) => {
      ...
      channel.bind("chart-data", (message) => {
        setChartData((prev) => {
          const next = [...prev, { timestamp: message.data.timestamp, requests: message.data.requests, errors: message.data.errors }]
      ...
      channel.bind("alert", (message) => {
        ...
        setAlerts((prev) => {
          const next = [...prev, { level: message.data.level, message: message.data.message, time }]
          return next.slice(-5)
        })
      })
    })
    ...
    <div className="max-w-lg mx-auto p-4 flex flex-col gap-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <StatCard
          ...
          previous={prevStats.errorRate !== null ? prevStats.errorRate * 100 : null}
        />
        <StatCard
          label="Avg Latency"
