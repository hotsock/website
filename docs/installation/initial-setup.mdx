...
Each stack can be created using either the AWS CLI (recommended) or the AWS Console. Use whichever you prefer. Although this document is detailed and lengthy, installation requires running just two commands in your account! Want to jump straight to it? [Here's the first](#installer-permissions-stack-cli) and [here's the second](#main-stack-cli).
...
1. Run the following command. Feel free to change the `--stack-name` value if you have other stack naming conventions, but keep other parameters as they are.
...
1. Switch to the desired (and [supported](./region-support.mdx)) AWS region using the region switcher in the top navigation.
1. Click the CloudShell icon in the top navigation bar or footer bar in the AWS Console.
1. Run the following command. Feel free to change the `--stack-name` value if you have other stack naming conventions (keep the name short though!), but keep other parameters as they are.
...
Once the stack creation completes, installation was successful. Next you'll likely want to [configure Authentication](../connection/authentication.mdx).
