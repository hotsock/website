...
Each member in a presence channel has a `uid` field to identify the user and a `umd` field to supply additional user data. These attributes are supplied by authentication or subscription token claims ([`uid`](../connection/claims.mdx#uid) and [`umd`](../connection/claims.mdx#umd)) and shared with all other members of the channel.
