...
## When to use storage {/* #when-to-use-storage */}
...
| [`scheduleBefore`](../connections/claims.mdx#channels.storage.scheduleBefore)   | The furthest future time this connection can schedule storage writes for                                       |
...
## Writing storage entries {/* #writing-storage-entries */}
...
### Server-side writes {/* #server-side-writes */}
...
### Scheduled writes {/* #scheduled-writes */}
...
## Reading storage entries {/* #reading-storage-entries */}
...
## Observing storage entries {/* #observing-storage-entries */}
...
If you need observers to be notified when a value expires, explicitly set the storage entry to `null` at the desired time — either by publishing a [scheduled storage write](#scheduled-writes) or by invoking a `hotsock.channelStorageSet` from your backend when the expiration should take effect.
