...
1. For the `SigningKey1EncodedParameter`, paste the Base64 encoded value you calculated in the previous step.

1. If your token issuer sets a `kid` header for this key, set `SigningKey1IDParameter` to this value. Otherwise leave it blank. If specified, JWTs signed with this key must include the `kid` header matching this value.

1. For the `SigningKey1MethodParameter`, choose your desired signing method. Ensure that the SHA algorithm (256, 384, or 512) matches what is set by your token issuer. You can verify this by inspecting the `alg` value in the header portion of an issued JWT.

1. We recommend setting `TokenAudienceParameter`, but it is not required. `hotsock` or `hotsock-eu-west-1` (your installation's region) are reasonable values here, and if set, all signed JWTs must include the `aud` claim set to the value specified in this parameter. This ensures that valid tokens signed for other services with the same private key cannot be unexpectedly used as a Hotsock token.

1. We recommend setting `TokenIssuerParameter`, but it is not required. `my-application-name` (your application) is a reasonable values here, and if set, all signed JWTs must include the `iss` claim set to the value specified in this parameter.

Leave all `SigningKey2*` parameters blank for now. Those secondary key parameters are for zero-downtime key rotation.
...
Each installation also includes a handy web console where you can ensure issued tokens work as expected. Grab the URL from the [WebConsoleHttpUrl](../installation/initial-setup.mdx#WebConsoleHttpUrl) stack output and paste a valid token to connect.
