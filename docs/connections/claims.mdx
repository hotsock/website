...
Each object inside the channels object accepts [`subscribe`](#subscribe) and [`sendMessages`](#sendMessages) attributes.
...
`Object` (optional) - Allows client-initiated messages to be sent directly to the WebSocket on the channel(s) in the object key for the specified events. Other channel members will receive these messages (if authorized).

The following allows this connection to subscribe to the channel "mychannel" and send client-initiated messages with the "isTyping" event name.
...
`Boolean` (optional) - If [sendMessages](#sendMessages) are permitted for this channel, the `echo` attribute specifies whether or not this connection that is sending these messages will receive a copy on the channel. Default is `false`.

In the following example, a copy of all `chat` client event messages will be sent to all subscribers, including the sender. `isTyping` client event messages will be sent to all other subscribers, but the sender will not receive a copy.
...
`Boolean` (optional) - If [client-initiated messages](#sendMessages) are permitted for this channel, the `emitPubSubEvent` attribute specifies whether or not these events will trigger a [backend pub/sub event to SNS/EventBridge](../server-api/events.mdx). Default is `false`. Enabling this setting has no effect if both SNS and EventBridge events are disabled globally.

In the following example, `chat` client event messages will trigger server-side pub/sub events. `isTyping` client event messages will not.
...
`Integer` (optional) - If [client-initiated messages](#sendmessages) are permitted for this channel, the `store` attribute specifies the number of seconds that written messages from this connection are retained in the database after being sent. `0` is the default, which does not save message data. `-1` keeps messages forever.

The following allows sending ephemeral "isTyping" events and stored "message.markAsRead" events to "mychannel", storing each "message.markAsRead" message for 30 days (30 days \* 24 hours \* 60 minutes \* 60 seconds) = 2,592,000 seconds.
...
`NumericDate` (optional) - The not-before claim specifies the earliest time that a token can be used, expressed as a Unix timestamp. This allows you to issue and distribute tokens prior to them being usable.
...
You can also specify multiple scopes in a single token.

```json
{
  "scope": "connect subscribe"
}
```
