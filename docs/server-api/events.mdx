...
- `keepAlive` (Boolean): Whether or not the [`keepAlive` claim](../connection/claims.mdx#keepAlive) is enabled for this connection.
- `sourceIp` (String | null): The source IP address that was used by the client when the connection was established.
- `uid` (String | null): The user ID specified in the [`uid` claim](../connection/claims.mdx#uid) for this connection.
- `umd` (JSON | null): The user metadata specified in the [`umd` claim](../connection/claims.mdx#umd) for this connection.
...
This event is sent whenever a message is published to Hotsock with `emitPubSubEvent` enabled. For server-initiated messages, set [`emitPubSubEvent`](./publish-messages.mdx#message-format--emitPubSubEvent) when publishing. For client-initiated messages, set the [`emitPubSubEvent` claim](../connection/claims.mdx#emitPubSubEvent) to `true` for desired channel events.
...
- `uid` (String | null): The user ID specified in the [`uid` claim](../connection/claims.mdx#uid) for this subscription.
- `umd` (JSON): The user metadata specified in the [`umd` claim](../connection/claims.mdx#umd) for this subscription.
