...
This event is sent whenever a message is published to Hotsock with `emitPubSubEvent` enabled. For server-initiated messages, set [`emitPubSubEvent`](./publish-messages.mdx#message-format--emitPubSubEvent) when publishing. For client-initiated messages, set the [`emitPubSubEvent` claim](../connections/claims.mdx#emitPubSubEvent) to `true` for desired channel events.
